...
export const LeversTable = ({ levers, partnerId, weekNumber, onUpdate, title }: LeversTableProps) => {
  ...
  const handleAddLever = async () => {
    ...
    if (error) {
      toast({ title: "Error", description: "Failed to add lever", variant: "destructive" });
    } else {
      setNewLever({ description: "", impactInput: "", confidenceInput: "", netImpact: 0 });
      onUpdate();
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b">
              <th className="text-left py-2 px-2 text-sm text-muted-foreground font-medium">Lever</th>
              <th className="text-center py-2 px-2 text-sm text-muted-foreground font-medium">Impact</th>
              <th className="text-center py-2 px-2 text-sm text-muted-foreground font-medium">Confidence</th>
              <th className="text-center py-2 px-2 text-sm text-muted-foreground font-medium">Net Impact</th>
              <th className="text-center py-2 px-2 text-sm text-muted-foreground font-medium">Actions</th>
            </tr>
          </thead>
          <tbody>
                ...
                <td className="py-2 px-2">
                  {editingLevers[lever.id] ? (
                    <Input
                      ...
                      onChange={(e) => setEditingLevers(prev => ({
                        ...prev,
                        [lever.id]: { ...prev[lever.id], description: e.target.value }
                      }))}
                      className="w-full"
                    />
                  ) : (
                    <span className="text-foreground text-sm">{lever.description}</span>
                  )}
                </td>
                <td className="py-2 px-2 text-center text-center placeholder:text-center">
                  {editingLevers[lever.id] ? (
                     <Input
                       type="number"
                       ...
                       onChange={(e) => {
                         ...
                         const netImpact = calculateNetImpact(impact, confidence);
                         setEditingLevers(prev => ({
                           ...prev,
                           [lever.id]: { ...prev[lever.id], impact, net_impact: netImpact }
                         }));
                       }}
                       className="w-20 mx-auto"
                     />
                  ) : (
                    <span className="text-foreground text-sm">{lever.impact}</span>
                  )}
                </td>
                <td className="py-2 px-2 text-center text-center placeholder:text-center">
                  {editingLevers[lever.id] ? (
                     <Input
                       ...
                       onChange={(e) => {
                         ...
                         setEditingLevers(prev => ({
                           ...prev,
                           [lever.id]: { ...prev[lever.id], confidence, net_impact: netImpact }
                         }));
                       }}
                       className="w-20 mx-auto"
                     />
                  ) : (
                    <span className="text-foreground text-sm">{lever.confidence}%</span>
                  )}
                </td>
                <td className="py-2 px-2 text-center">
                  {editingLevers[lever.id] ? (
                    <Input
                      ...
                      onChange={(e) => setEditingLevers(prev => ({
                        ...prev,
                        [lever.id]: { ...prev[lever.id], net_impact: Number(e.target.value) }
                      }))}
                      className="w-20 mx-auto"
                    />
                  ) : (
                    <span className="text-foreground">{lever.net_impact || 0}</span>
                  )}
                </td>
                <td className="py-2 px-2 text-center">
                  {editingLevers[lever.id] ? (
            ...
            <tr className="border-b">
              <td className="py-2 px-2">
                <Input
                  ...
                  onChange={(e) => setNewLever(prev => ({ ...prev, description: e.target.value }))}
              ...
              <td className="py-2 px-2 text-center">
                <Input
                  ...
                  onChange={(e) => setNewLever(prev => ({ ...prev, net_impact: Number(e.target.value) }))}
            ...
            <tr className="border-t-2 border-primary/20 bg-muted/30">
              ...
              <td className="py-2 px-2 text-center font-semibold text-sm text-foreground">
                {weekLevers.reduce((sum, lever) => sum + (lever.net_impact || 0), 0)}
              </td>
              <td className="py-2 px-2 text-center">
                -
              </td>
